...
int main(int ac, char *av[])
{
	...
	if (std::is_sorted(algorithm.getVector().begin(), algorithm.getVector().end()))
		std::cout << GREEN << "\nfordJohnson [std::vector] SUCCESSFUL" << WHITE;
	else
		std::cout << RED << "\nfordJohnson [std::vector] FAIL" << WHITE;
	if (std::is_sorted(algorithm.getDequeue().begin(), algorithm.getDequeue().end()))
		std::cout << GREEN << "\nfordJohnson [std::deque ] SUCCESSFUL" << std::endl << WHITE;
	else
		std::cout << RED << "\nfordJohnson [std::deque]  FAIL" << std::endl << WHITE;
#endif
	return (0);
}
